			...
			if(sockets_polled[fd_index].revents & POLLIN){
				if(fd_index==0){
					...
					if(num_read==0){ //client has hung up
						...
						read(sockets_polled[fd_index].fd, &ch, sizeof(ch));
						...
						*/
						...
						printf("file descriptor obtained%d\n",read_fd);						
						if(read_fd>1){						
							read(read_fd,&fbuf,sizeof(fbuf));
							write(sockets_polled[fd_index].fd, &fbuf,sizeof(fbuf));
						}					
					}
				}			
			}
